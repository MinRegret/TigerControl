...
def _verify_valid_environment(environment_class):
    ''' 
    Description: verifies that a given class has the necessary minimum environment methods
    ...
    for f in ['initialize', 'step']:
        if not callable(getattr(environment_class, f, None)):
            raise error.InvalidClass("CustomEnvironment is missing required method \'{}\'".format(f))

def register_custom_environment(custom_environment_class, custom_environment_id):
    '''
    Description: global custom environment method
    '''
