...
class LDS(ControlProblem):
    """
    Description: The base, master LDS class that all other LDS subproblems inherit. 
...
Methods:
    ...
    initialize(n, m, d, noise=1.0)
        ...
        Args:
            ...
            partially_observable (bool): whether to project x to y
            noise_distribution (None, string, OR func): if None then no noise. Valid strings include ['normal', 'uniform']. 
                Valid noise functions must map a subset of the following permissible inputs: n (x dim), x (state), u (action), 
                w (previous noise), and t (current time) to either a scalar or an n-dimensional vector of real values.
                Ex: noise_distribution = lambda x, t: 0.1 * x + np.cos(t)
            noise magnitude (float): magnitude of noise
