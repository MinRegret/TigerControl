# test the KalmanFilter method class
...
def test_kalman_filter(steps=100, show_plot=True):
    ...
    method = tigercontrol.method("KalmanFilter")
    method.initialize(x0, 1, 0, 1, 1, 0, env_noise)
    ...
    for i in range(T):
        ...
        x_pred = method.step(0, z)
        cur_loss = float(loss(x_true, x_pred))
        results.append(cur_loss)

    if show_plot:
        plt.plot(results)
        plt.title("KalmanFilter method on constant signal")
