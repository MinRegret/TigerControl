...
def test_new_experiment(steps=5000, show=False):
    ...
    '''exp.add_model('LSTM', {'n' : 1, 'm' : 1, 'l' : 16, 'h' : 32, 'optimizer' : OGD}, name = '16-32-OGD')
                exp.add_model('LSTM', {'n' : 1, 'm' : 1, 'l' : 32, 'h' : 64, 'optimizer' : OGD}, name = '32-64-OGD')
            
                exp.add_model('LSTM', {'n' : 1, 'm' : 1, 'l' : 16, 'h' : 32, 'optimizer' : Adagrad}, name = '16-32-Adagrad')
                exp.add_model('LSTM', {'n' : 1, 'm' : 1, 'l' : 32, 'h' : 64, 'optimizer' : Adagrad}, name = '32-64-Adagrad')
            
                exp.add_model('LSTM', {'n' : 1, 'm' : 1, 'l' : 16, 'h' : 32,  'optimizer' : ONS}, name = '16-32-ONS')
                exp.add_model('LSTM', {'n' : 1, 'm' : 1, 'l' : 32, 'h' : 64,  'optimizer' : ONS}, name = '32-64-ONS')
            
                exp.add_model('LSTM', {'n' : 1, 'm' : 1, 'l' : 16, 'h' : 32,  'optimizer' : Adam}, name = '16-32-Adam')
    ...
    datapath = 'data/results/scoreboard_real_RNN.csv'
    datapath = os.path.join(ctsb_dir, datapath)

    exp.scoreboard(save_as = datapath)
    #exp.scoreboard(metric = 'time')

    '''datapath = 'data/results/graph_simulated.png'
                datapath = os.path.join(ctsb_dir, datapath)
            
                exp.graph(problem_ids = ['ARMA-20', 'ARMA-33', 'ARMA-54', \
                    'LDS-TimeSeries-3', 'LDS-TimeSeries-10', 'LSTM-TimeSeries-32', 'LSTM-TimeSeries-64'], \
                    yscale = 'log', save_as = datapath, dpi = 300)'''

    datapath = 'data/results/graph_real_RNN.png'
    datapath = os.path.join(ctsb_dir, datapath)
