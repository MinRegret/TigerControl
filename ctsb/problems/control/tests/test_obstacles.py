...
def compute_fgm_angle(y, thetas_nominal, cur_angle):
    ...
    threshold_filter = lambda x : x >= 5.0
    y_filter = list(map(threshold_filter, y[0]))
    y_filter.append(False)
    run, best_run = 0, 0
    ...
    #deviation_mapper = lambda angle : abs(angle - cur_angle)
    deviation_mapper = lambda angle : abs(angle - 0.0)
    deviations = list(map(deviation_mapper, best_angles))

    # print("deviations : " + str(deviations))
    min_dev_angle = best_angles[deviations.index(min(deviations))] if best_angles != [] else cur_angle

    print("min_dev_angle: " + str(min_dev_angle))
    return min_dev_angle
